...
def CameraPoseRead(dir, index):
    '''
    Takes in index and npy file, outputs the particular pose w.r.t
    base frame
    '''
    camera_pose = dir[index]
    return camera_pose
...
if __name__ == '__main__':
    ...
    scene = '0001'
    root = '/home/jay/Desktop/Neural-Point-Cloud-Rendering'
    dir1 = '/home/jay/Documents/graspwork/graspnet/scenes/scene_0000/kinect/depth/'#'../data/ScanNet/%s/depth/' % scene
    dir2 = '/home/jay/Documents/graspwork/graspnet/scenes/scene_0000/kinect/camK.npy'#'../data/ScanNet/%s/intrinsic/' % scene
    dir3 = np.load('/home/jay/Documents/graspwork/graspnet/scenes/scene_0000/kinect/camera_poses.npy')#'../data/ScanNet/%s/pose/' % scene
    dir4 = '/home/jay/Documents/graspwork/graspnet/scenes/scene_0000/kinect/rgb/'#'../data/ScanNet/%s/color/' % scene
    point_clouds_path = 'example_pcd.ply'#/home/jay/Documents/graspwork/augment_files/scene1/scene1part_0.pcd'#'../pre_processing_results/ScanNet/%s/point_clouds_simplified.ply' % scene
    output_dir = root+'/pre_processing_results/Graspnet/%s/reproject_results_%s/' % (scene, num)
    output_dir1 = root+'/pre_processing_results/Graspnet/%s/weight_%s/' % (scene, num)
...
######################################################################################################################
    ...
    ##todo : load point_clouds and point_clouds_colors for graspnet
    #point_clouds, point_clouds_colors = loadfile(point_clouds_path)
    pcd = o3d.io.read_point_cloud(point_clouds_path)
    point_clouds = np.transpose(np.asarray(pcd.points))
    newrow = np.ones([point_clouds.shape[1]])
    point_clouds = np.vstack([point_clouds, newrow])
    point_clouds_colors = np.transpose(np.asarray(pcd.colors))*255.0

    ##ToDo - change cameraparam read function to get intrinisc color and depth for graspnet
    #intrinsic_color, intrinsic_depth, _, _ = CameraParameterRead(dir2)
    intrinsic_color = np.load(dir2)
    intrinsic_color = np.vstack([intrinsic_color,[0,0,0]])
    #print(intrinsic_color)
    intrinsic_color = np.hstack([intrinsic_color,[[0],[0],[0],[1]]])    
    if resize:
